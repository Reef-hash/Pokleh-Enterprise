  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-xl sm:text-2xl font-bold tracking-tight">{t('supplier.settlements-title')}</h2>
          <p className="text-muted-foreground">{t('supplier.settlements-subtitle')}</p>
        </div>
      </div>
      ...
      {userRole === "admin" && unsettledIntakes.length > 0 && (
        <Card>
          <CardHeader>
            <CardTitle>{t('supplier.pending-calculations')}</CardTitle>
            <CardDescription>{t('supplier.intakes-need-calculation')}</CardDescription>
          </CardHeader>
              ...
              <div key={i.id} className="flex items-center justify-between p-3 border rounded-lg">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    {new Date(i.intake_date).toLocaleDateString()} — {i.quantity_received} {t('common.pax')} @ {formatCurrency(i.cost_per_pax)}
                  </p>
                </div>
                <Button size="sm" onClick={() => calculateSettlement(i.id)}>
                  <Calculator className="mr-2 h-4 w-4" /> {t('supplier.calculate')}
                </Button>
              </div>
            ))}
          </CardContent>
      ...
      <Card>
        <CardHeader>
          <CardTitle>
            {t('supplier.settlements')} ({settlements.length})
            {totalPayable > 0 && (
              <span className="ml-4 text-sm font-normal text-muted-foreground">
                {t('supplier.total-pending')}: {formatCurrency(totalPayable)}
              </span>
            )}
          </CardTitle>
        </CardHeader>
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead>{t('stock.intake-date')}</TableHead>
                  <TableHead>{t('supplier.received')}</TableHead>
                  <TableHead>{t('supplier.sold')}</TableHead>
                  <TableHead>{t('supplier.wasted')}</TableHead>
                  <TableHead>{t('supplier.reduction')}</TableHead>
                  <TableHead>{t('supplier.returned')}</TableHead>
                  <TableHead>{t('supplier.payable-qty')}</TableHead>
                  <TableHead>{t('supplier.cost-per-pax')}</TableHead>
                  <TableHead>{t('supplier.payable-rm')}</TableHead>
                      ...
                      <TableCell>
                        {s.status === "pending" && (
                          <Button size="sm" variant="outline" onClick={() => markSettled(s.id)}>
                            <CheckCircle className="mr-1 h-4 w-4" /> {t('supplier.settle')}
                          </Button>
                        )}
                      </TableCell>
                    )}
          ...
          <div className="block md:hidden space-y-3">
            {settlements.map((s) => (
              <ResponsiveCard key={s.id}>
                ...
                <ResponsiveRow label={t('stock.intake-date')}>
                  {s.intake ? new Date(s.intake.intake_date).toLocaleDateString() : "—"}
                </ResponsiveRow>
                <ResponsiveRow label={t('supplier.received')}>{s.total_received}</ResponsiveRow>
                <ResponsiveRow label={t('supplier.sold')}>{s.total_sold}</ResponsiveRow>
                <ResponsiveRow label={t('supplier.wasted')}>
                  <span className="text-orange-600 font-medium">{s.total_wastage || 0}</span>
                </ResponsiveRow>
                {(s.wastage_reduction || 0) > 0 && (
                  <ResponsiveRow label={t('supplier.reduction')}>
                    <span className="text-green-600">-{s.wastage_reduction || 0}</span>
                  </ResponsiveRow>
                )}
                <ResponsiveRow label={t('supplier.returned')}>{s.total_returned}</ResponsiveRow>
                <ResponsiveRow label={t('supplier.payable-qty')}>
                  <span className="font-medium">{s.payable_quantity}</span>
                </ResponsiveRow>
                <ResponsiveRow label={t('supplier.cost-per-pax')}>{formatCurrency(s.cost_per_pax)}</ResponsiveRow>
                <ResponsiveRow label={t('supplier.payable-rm')}>
                ...
                {userRole === "admin" && s.status === "pending" && (
                  <ResponsiveRow label={t('common.action')}>
                    <Button size="sm" variant="outline" onClick={() => markSettled(s.id)}>
                      <CheckCircle className="mr-1 h-4 w-4" /> {t('supplier.settle')}
                    </Button>
                  </ResponsiveRow>
